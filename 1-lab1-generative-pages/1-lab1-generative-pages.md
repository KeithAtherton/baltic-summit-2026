...
1. Let's check out some of the options:
    ...
    - Tools, Include images: At the bottom-left of the prompt box, select the Tools button to see the *Include images* toggle. Toggling this on (it should be toggled on by default) allows the agent to access images from a library of 25,000 stock images for things like placeholder images and decorative backgrounds.
...
1. In the bottom-left, select +, *Add table*, select *Innovation Idea*.

    - If doing this adds "Innovation Idea" to the end of your prompt, go ahead and delete it.

1. Select the *Generate* button in the bottom-right of the prompt box. It may take a while to complete.
...
1. Again, AI can get things wrong (and often does!). If it does, that's expected and ok, we can try to fix it. For example, if the agent generated the page but the cards do not contain the idea *Name*, try a prompt to fix it. For example:
...
> [!NOTE]
> Again, if the feature does not work as expected then AI has missed something. Try more prompts to address any issues.
